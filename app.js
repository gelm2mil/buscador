...
// === MAPA PROFESIONAL DE ALIAS DE CAMPOS === //
const aliases = {
    placa: ["placa", "placas", "no placa", "num placa", "número de placa", "placa no", "p l a c a"],
    tipo: ["tipo", "vehiculo", "clase"],
    marca: ["marca", "carro", "motocicleta", "vehiculo marca"],
    color: ["color"],
    direccion: ["direccion", "dir", "ubicacion", "lugar"],
    conductor: ["conductor", "nombre", "nombre del conductor", "piloto"],
    licencia: ["licencia", "no licencia", "num licencia", "numero licencia"],
    articulo: ["articulo", "art", "faltas", "articulo ley"],
    chapa: ["chapa", "número chapa", "no chapa"]
};

// === FUNCIÓN PARA ENCONTRAR COLUMNAS SIN IMPORTAR EL NOMBRE === //
function matchField(key) {
    const clean = key.toLowerCase().replace(/[^a-z0-9]/g, "");

    for (let field in aliases) {
        for (let alias of aliases[field]) {
            const cleanAlias = alias.toLowerCase().replace(/[^a-z0-9]/g, "");
            if (clean.includes(cleanAlias)) return field;
        }
    }
    return key; // deja todo lo que no esté mapeado
}

document.getElementById("buscarBtn").addEventListener("click", function () {
    const input = document.getElementById("placaInput").value.trim().toUpperCase();
    const resultDiv = document.getElementById("resultado");

    if (!window.placasData) {
        resultDiv.innerHTML = "<p style='color:red'>El archivo todavía está cargando...</p>";
        return;
    }

    const placaNormalizada = input.replace(/[\s-]/g, "");

    // === BUSCAR LA PLACA EN CUALQUIER COLUMNA RELACIONADA === //
    const registro = window.placasData.find(item => {
        for (let key in item) {
            const campo = matchField(key);
            if (campo === "placa") {
                const valor = String(item[key]).toUpperCase().replace(/[\s-]/g, "");
                if (valor === placaNormalizada) return true;
            }
        }
        return false;
    });
    ...
    if (!registro) {
        resultDiv.innerHTML = "<p style='color:red'>Placa no encontrada.</p>";
        return;
    }

    // === CONVERTIR LAS KEYS A CAMPOS PROFESIONALES === //
    const datosLimpios = {};
    for (let key in registro) {
        const campo = matchField(key);
        datosLimpios[campo] = registro[key];
    }

    // === MOSTRAR TODO ORDENADO Y PROFESIONAL === //
    let html = "<div class='resultado-card'>";
    for (let campo in datosLimpios) {
        html += `<p><strong>${campo.toUpperCase()}:</strong> ${datosLimpios[campo]}</p>`;
    }
    html += "</div>";
